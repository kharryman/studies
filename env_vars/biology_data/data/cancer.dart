final dynamic cancerBio = {
  "index": 12,
  "itemName": "Cancer Biology",
  "imageFolder": "CANCER_BIOLOGY",
  "folderName": "Cancer Biology",
  "type": "Molecular",
  "entries": [
    {
      "title": "Cancer",
      "type": "NORMAL",
      "image": null,
      "data": [
        {
          "name": null,
          "value":
              "defini­tions of cancer:\n• neoplasm of prolif­erating cells (new tissue growing out of control)\n• cell division out of control\n• cells produced are useless\n• compete with healthy cells for nutrients and oxygen\nbenigns (not moving) versus malignant (moving)\n• benign is a sedentary mass of cancerous cells\n• malignant is a moving mass of cancerous cells - metastasis"
        }
      ]
    },
    {
      "title": "Causes of Cancer",
      "type": "NORMAL",
      "image": null,
      "data": [
        {
          "name": null,
          "value":
              "•mutations\n•carcin­ogenic chemicals\n•UV light\nviruses\n•oncogenes\n•p53 genes"
        }
      ]
    },
    {
      "title": "Basic Types of Cancer",
      "type": "NORMAL",
      "image": null,
      "data": [
        {
          "name": "Carcinoma",
          "value": "Arises from body's outer coverings and inner linings."
        },
        {
          "name": "Sarcoma",
          "value": "Arises from body's supporting structures."
        },
        {"name": "Lymphoma", "value": "Arises from lymph system."},
        {"name": "Leukemia", "value": "Arises from red marrow in bone, spleen."}
      ]
    },
    {
      "title": "Cancer (from mutations in cell cycle)",
      "type": "NORMAL",
      "image": null,
      "data": [
        {
          "name": null,
          "value":
              "Cancer cells have large nuclei, no use, they attract blood vessels and become tumours, and can metast­asize"
        }
      ]
    },
    {
      "title": "Disrupt Cell Cycle and Trigger Cancer",
      "type": "NORMAL",
      "image": null,
      "data": [
        {
          "name": null,
          "value":
              "Genes that stop or slow the cell cycle\n• ex: tumor suppressor genes\ngenes that trigger cell growth and division by initiating different stages of the cell cycle\n• ex: proto-­onc­ogenes"
        }
      ]
    },
    {
      "title": "What Causes Uncont­rolled Cell Growth?",
      "type": "NORMAL",
      "image": null,
      "data": [
        {
          "name": null,
          "value":
              "Defects in proteins that control the cell cycle\nmutations that knock out key genes"
        }
      ]
    },
    {
      "title": "Proto onc­ogenes",
      "type": "NORMAL",
      "image": null,
      "data": [
        {
          "name": null,
          "value":
              "•Initiate each phase of the cell cycle\n•Active when conditions are approp­riate for growth\n•Mutations cause growth to occur at all times\n•Oncogenes promote cancer develo­pment"
        }
      ]
    },
    {
      "title": "Tumor ­Sup­pressor Genes",
      "type": "NORMAL",
      "image": null,
      "data": [
        {
          "name": null,
          "value":
              "•p53 (guardian of the genome)\n• p53 protein increases in cells exposed to UV radiation\n•p53 mode of action­:\n1) DNA damage\n2) p53 increases\n3) p53 bind to DNA (not at damage site)\n4) transc­ription of genes that stop cell cycle or lead to cell death"
        }
      ]
    },
    {
      "title": "Chemotherapy",
      "type": "NORMAL",
      "data": [
        {"name": null, "value": "The use of drugs to treat a disease."}
      ]
    },
    {
      "title": "Chemot­her­apeutic Spectra of Antiba­cterial Agents",
      "type": "TABLE_LIST",
      "data": [
        {
          "name": "Narrow Spectrum",
          "value":
              "Prefer­ent­ially active against single or limited group of microo­rga­nisms\nTx eg: isoniazid"
        },
        {
          "name": "Extended Spectrum",
          "value":
              "Effective against gram-p­ositive and SOME gram negative bacteria\nTx eg: ampicillin"
        },
        {
          "name": "Broad Spectrum",
          "value":
              "Active against BOTH gram positive and gram negative bacteria\nTx eg: tetrac­ycline\nTx eg: chlora­mph­enicol"
        }
      ]
    },
    {
      "title": "Types of Pathways",
      "type": "TABLE_LIST",
      "data": [
        {
          "name": "Unique Pathways",
          "value":
              "Also known as Cell Wall Synthesis Inhibi­tors; drug that inhibits the cell wall synthesis in microbes; the walls will lyse and the bacteria will die"
        },
        {
          "name": "Selective Pathways",
          "value": "Also known as protein synthesis inhibitors"
        },
        {"name": "Common Pathways", "value": "Also known as metabo­lites"}
      ]
    },
    {
      "title": "Selective Toxicity",
      "type": "TABLE_LIST",
      "data": [
        {
          "name": "Definition",
          "value":
              "Ability to kill or injure an invading microo­rganism without harming the cells of the host"
        },
        {"name": "LD50", "value": "Lethal dose at 50%; should be high"},
        {
          "name": "MIC",
          "value":
              "Minimal inhibitory concen­tra­tion; should be low; the lowest concen­tration of antibiotic that INHIBITS bacterial growth; lowest concen­tration that will stop the growth of bacteria"
        },
        {
          "name": "MBC",
          "value":
              "Minimal bacter­icidal concen­tra­tion; should be; minimum concen­tration of antibiotic that KILLS the bacteria"
        }
      ]
    },
    {
      "title": "Types of Bacterial Infections",
      "type": "NORMAL",
      "image": "BACTERIAL_INFECTIONS",
      "data": [
        {"name": null, "value": null}
      ]
    },
    {
      "title": "Acid-Fast Bacteria",
      "type": "NORMAL",
      "image": null,
      "data": [
        {
          "name": null,
          "value": "•Don't retain crystal violet dye from gram staining"
        },
        {"name": null, "value": "•They are pink or red colored"},
        {"name": null, "value": "•Thin peptid­oglycan wall"},
        {"name": null, "value": "•Two phosph­olipid bilayers (two membranes)"},
        {
          "name": null,
          "value":
              "•Consist of outer membrane and thin peptid­oglycan wall as cell wall"
        },
        {"name": null, "value": "•The cell wall is thinner than gram positive"},
        {
          "name": null,
          "value":
              "•This is harder to treat with antibi­otics because it has two phosph­olipid bilayers"
        }
      ]
    },
    {
      "title": "Site of Action of Antiba­cterial Drug Classes",
      "type": "TABLE_LIST",
      "data": [
        {
          "name": "Cell Wall Inhibi­tors",
          "value":
              "Prefer­ent­ially active against single or limited group of microo­rga­nisms\nTx eg: isoniazid"
        },
        {
          "name": "DNA Synthesis & Integrity Inhibi­tors",
          "value": "Sulfon­amides\nTrimet­hoprim\nQuinolones"
        },
        {
          "name": "Tran­scr­iption & Translation Inhibi­tors",
          "value":
              "Rifampin\nAxmino­gly­cosdes\nSpecti­nomycin\nTetrac­yclines\nMacrolides\nChlora­mph­enicol\nStrept­ogr­amins\nOxazol­idi­nones"
        }
      ]
    },
    {
      "title": "Site of Action of Antiba­cterial Drug Classes",
      "type": "TABLE_LIST",
      "data": [
        {
          "name": "Cell Wall Inhibi­tors",
          "value":
              "Prefer­ent­ially active against single or limited group of microo­rga­nisms\nTx eg: isoniazid"
        },
        {
          "name": "DNA Synthesis & Integrity Inhibi­tors",
          "value": "Sulfon­amides\nTrimet­hoprim\nQuinolones"
        },
        {
          "name": "Tran­scr­iption & Translation Inhibi­tors",
          "value":
              "Rifampin\nAxmino­gly­cosdes\nSpecti­nomycin\nTetrac­yclines\nMacrolides\nChlora­mph­enicol\nStrept­ogr­amins\nOxazol­idi­nones"
        }
      ]
    },
    {
      "title": "Site of Action of Antiba­cterial Drug Classes",
      "type": "NORMAL",
      "image": "ANTIBACTERIAL_DRUG_CLASSES",
      "data": []
    },
    {
      "title": "Gram NEGATIVE Bacteria",
      "type": "NORMAL",
      "image": null,
      "data": [
        {
          "name": "Defi­nit­ion",
          "value":
              "Bacteria which resist decolo­riz­ation with an acid-a­lcohol mixture during the acid-fast stain procedure."
        },
        {
          "name": null,
          "value":
              "•It retains the initial dye (carbo­fuc­hsin)\n•Acid-fast bacteria (mycob­acteria and some of the related actino­myc­etes) appear red"
        }
      ]
    },
    {
      "title": "Gram POSTIVE Bacteria",
      "type": "NORMAL",
      "image": null,
      "data": [
        {
          "name": null,
          "value":
              "•Looks violet or dark blue in gram staining\n•Retains the crystal violet stain\n•Single layered membrane -- it lacks the second outer phosph­olipid bilayer\n•Thick layer of peptid­oglycan -- only this forms the cell wall\n•Easier to treat with antibi­otics because it only has one phosph­olipid bilayer"
        }
      ]
    },
    {
      "title": "Bacteria Morphology",
      "type": "TABLE_LIST",
      "image": null,
      "data": [
        {"name": "Spirilla", "value": "Spiral shaped bacteria"},
        {"name": "Bacilla", "value": "Rod shaped bacteria"},
        {"name": "Cocci", "value": "Sperical shaped bacteria"},
        {"name": "Diplo-", "value": "Pair"},
        {"name": "Staphylo-", "value": "Clusters"},
        {"name": "Strepto-", "value": "Chain"}
      ]
    },
    {
      "title": "Bacterialstatic Drugs",
      "type": "NORMAL",
      "image": null,
      "data": [
        {
          "name": null,
          "value":
              "•INHI­BIT the growth of pathogens without causing cell death\n•Eg: sulfon­amides (DNA synthesis & intercity inhibitor)\nEg: chlora­mph­enicol (trans­cri­ption & transl­ation inhibitor)\n•Bacter­ios­tatic effect­iveness relies on an intact host immune system to CLEAR THE NONGROWING (but viable) bacteria"
        }
      ]
    },
    {
      "title": "Types of Antibiotic Agents",
      "type": "TABLE",
      "headers": ["Type", "Example"],
      "data": [
        {
          "columns": [
            {"value": "Cause inhibition of cell wall synthesis"},
            {"value": "Beta-L­actamas"}
          ]
        },
        {
          "columns": [
            {
              "value":
                  "Alter the function of the cytopl­asmic membrane; destroy cytopl­asmic membranes"
            },
            {"value": "Isoniazid"}
          ]
        },
        {
          "columns": [
            {"value": "Inhibit protein synthesis"},
            {"value": "Macrolides"}
          ]
        },
        {
          "columns": [
            {"value": "Inhibit nucleic acid synthesis"},
            {"value": "Quinolones"}
          ]
        },
        {
          "columns": [
            {"value": "Inhibit metabolite activity"},
            {"value": "Sulfon­amides"}
          ]
        }
      ]
    },
    {
      "title": "Antibiotics",
      "type": "NORMAL",
      "data": [
        {
          "name": null,
          "value":
              "•Substances produced by a microo­rganism that (at low concen­tra­tion) inhibit or kill other microo­rga­nisms\n•Chemicals produce by living organisms that can kill or inhibit\n•BASI­CAL­LY: life destroys life"
        }
      ]
    },
    {
      "title": "Antibi­otics in Foods",
      "type": "NORMAL",
      "image": null,
      "data": [
        {
          "name": null,
          "value":
              "•Antibi­otics are used in animal feeds and sprayed on plants to prevent infection and promote growth.\n•Multi-drug resistant Salmo­nella typhi has been found in some people who eat beef fed antibi­otics"
        }
      ]
    },
    {
      "title": "Prophylactic Antibiotics",
      "type": "NORMAL",
      "image": null,
      "data": [
        {
          "name": null,
          "value":
              "•Use of antibi­otics for prevention instead of treatment of infection\n•May cause resistance and superi­nfe­ction\n•Use is limited"
        }
      ]
    },
    {
      "title": "Compli­cations of Antibiotic Therapy",
      "type": "NORMAL",
      "image": null,
      "data": [
        {
          "name": null,
          "value":
              "1. Hypers­ens­itivity\n2. Direct toxicity\n3. Superi­nfe­ction"
        }
      ]
    },
    {
      "title": "Result of Acquired Antibiotic Resistance",
      "type": "NORMAL",
      "image": null,
      "data": [
        {
          "name": null,
          "value":
              "1. Sponta­neous, random chromo­somal mutations. The mutations are due to change in either a structural protein receptor for an antibiotic or a protein involved in drug transport\n2. Extrac­hro­mosomal transfer of drug-r­esi­stant genes\n2a. Tran­sfo­rma­tion: transfer of naked DNA between cells of same species\n2b. Tran­sdu­ction through R plasmi­ds: R plasmids are a sexual transfer of plasmid DNA in a bacteria virus between bacteria of the same species\n2c. Conj­uga­tion: the passage of gene from bacteria to bacteria via direct contact through a sex plus or bridge. Conjug­ation occurs primarily in GRAM NEGATIVE BACILLI. It is the principal mechanism of acquired resistant among entero­bac­teria\n2d. Tran­spo­sit­ion: occurs as a result of movement or \"­jumping or transp­oso­ns\" (stretches of DNA containing insertion sequences at each end) from plasmid to plasmid or from plasmid to chromosome and back"
        }
      ]
    },
    {
      "title": "Antimicrobial Drugs",
      "type": "NORMAL",
      "data": [
        {
          "name": null,
          "value":
              "Any substance of natural, synthetic or semi-s­ynt­hetic origin that kills or inhibits the growth of a microo­rganism. Causes little or no host damage"
        }
      ]
    },
    {
      "title": "Combinations of Antimicrobrial Drugs",
      "type": "TABLE_LIST",
      "image": null,
      "data": [
        {
          "name": "Advantage:",
          "value": "Synergism\nEg: beta-l­actams and aminog­lyc­osides"
        },
        {
          "name": "Disadvantage:",
          "value":
              "Drug antagonism\nEg: combining bacter­ios­tatic drug with bacter­icidal drug\nEg: giving a patient tetrac­ycline with penicillin or cephal­osp­orins"
        },
        {
          "name": "BASICALLY:",
          "value":
              "Don't combine bacter­ios­tatic drugs with bacter­icidal drugs"
        }
      ]
    },
    {
      "title": "Selection of Antimi­crobial Agents",
      "type": "NORMAL",
      "data": [
        {
          "name": null,
          "value":
              "Requires knowing the following:\n•The organism's identity\n•The organism's suscep­tib­ility to a particular agent\n•The site of infection\n•Patient's factors\n•The safety of the agent\n•The cost of therapy"
        }
      ]
    },
    {
      "title": "Inappr­opriate Antimi­crobial Use",
      "type": "NORMAL",
      "image": null,
      "data": [
        {
          "name": null,
          "value":
              "•Prescr­iptions not taken correctly\n•Antibi­otics for viral infections (you don't give antibi­otics for viral infect­ions)\n•Antibi­otics sold without medical superv­ision\n•Spread of resistant microbes in hospitals due to lack of hygiene\n•Lack of quality control in manufa­cture of outdated antimi­crobial\n•Inadequate survei­llance of defective suscep­tib­ility assays\n•Poverty or way\n•Use of antibi­otics in foods"
        }
      ]
    },
    {
      "title": "Antimi­crobial Resistance",
      "type": "NORMAL",
      "image": null,
      "data": [
        {
          "name": "Defi­nit­ion",
          "value":
              "Relative or complete lack of effect of antimi­crobial against a previously suscep­tible microbe."
        },
        {
          "name": null,
          "value":
              "•Increase in MIC (remember MIC is lowest concen­tration needed to inhibit bacterial growth)\n•May be innate (an escape from antibiotic effect)\n•OR it may be acquired"
        }
      ]
    },
    {
      "title": "Mechanisms of Antimi­crobial Resistance",
      "type": "NORMAL",
      "image": null,
      "data": [
        {
          "name": null,
          "value":
              "1. Reduced entry of antibiotic into pathogen\n2. Enhanced export of antibiotic by pathogen efflux pumps\n3. release of microbial enzymes that destroy the antibiotic\n4. Altera­tions of microbial enzymes that are required to transform products to the effective moieties\n4. Altera­tions of target proteins\n5. Develo­pment of altern­ative bioche­mical pathways to those inhibited by the antibiotic"
        }
      ]
    },
    {
      "title": "Factors that Promote Antimi­crobial Resistance",
      "type": "NORMAL",
      "image": null,
      "data": [
        {
          "name": null,
          "value":
              "1. Exposure to sub-op­timal levels of antimi­crobial\n2. Exposure to microbes carrying resistance genes"
        }
      ]
    },
    {
      "title":
          "Reasons of Using Single Drug to Treat a Patient and Not Combinations of Antimicrobrial Drugs",
      "type": "NORMAL",
      "image": null,
      "data": [
        {
          "name": null,
          "value":
              "1. Reduces the possib­ility of superi­nfe­ction\n2. Reduces the emergence of resistant organisms\n3. Minimizes toxicity"
        }
      ]
    },
    {
      "title": "Medically Important Microo­rga­nisms",
      "type": "NORMAL",
      "image": null,
      "data": [
        {
          "name": null,
          "value":
              "•Gram Positive Cocci\n•Gram Positive Bacilli\n•Gram Positive Cocci\n•Gram Negative Bacilli\n•Anaerobe Organisms\n•Spiroc­hetes\n•Mycoplasma\n•Chlamydia\n•Other"
        }
      ]
    },
    {
      "title": "Mechanism of Selective Targeting",
      "type": "NORMAL",
      "data": [
        {
          "name": null,
          "value":
              "•Sele­ctive Toxici­ty: goal of antimi­crobial drug therapy\n•Exam­ple: inhibit pathways or targets critical for pathogen survival at drug concen­tra­tions lower than those required to affect host pathways"
        }
      ]
    },
    {
      "title":
          "MRSA \"­mer­-sa­h\"(Meth­ici­lli­n-R­esi­stant Staphy­loc­occus Aureus)",
      "type": "NORMAL",
      "image": null,
      "data": [
        {
          "name": null,
          "value":
              "Most frequent nosocomial (hospi­tal­-ac­quired) pathogen. Usually resistant to several other antibi­otics"
        }
      ]
    }
  ]
};
